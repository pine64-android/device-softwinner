...
function sysconfig_CreateImgName() {
	...
	if [ "$myTP" == "1" ]; then
		...
		myTP="_rtp"##$(sysconfig_getValue $fexCfgFile rtp_used)
	fi
	...
	local MyImgName=`echo -e ${fexCfgDirName}_${myChipSName}_${myAllwinnerVer}_${myAndroidVer}_${TIME_Name}01_[${myLcdBitwidth}${myLcdX}x${myLcdY}${myTP}${myCamera}${myGsensor}$myDDR$myOdt${myMotor}${myBTooth}${myStorageType}${myImgDebugName}]_.img`
	echo $MyImgName
}
